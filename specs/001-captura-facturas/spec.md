...
- **FR-020**: El sistema MUST detectar duplicados probables: dos documentos sin CUFE, o cuyo CUFE proviene de OCR de respaldo en vez de un código QR legible (ver FR-006/edge case de QR ilegible — un CUFE leído así no está garantizado carácter por carácter), cuya fecha y total coinciden exactamente y cuyo nombre de comercio es similar (tolerante a variaciones menores producto de la extracción, no exige coincidencia carácter por carácter), MUST quedar marcados como posible duplicado, y el sistema MUST pedir al usuario que confirme si representan la misma compra, en lugar de decidirlo automáticamente. Un CUFE leído desde un código QR se mantiene fuera de este mecanismo (FR-019 ya lo cubre con coincidencia exacta automática).
...
- El umbral de "baja confianza" (FR-009) que dispara `necesita_revisión` se fija en **0.7** (campos con confianza estrictamente menor). Es un valor inicial razonable, no derivado de una medición con facturas reales todavía; ajustar en `packages/domain/src/tax-rules/cuadre-monetario.ts` si la validación con el backlog real muestra que es demasiado laxo o demasiado estricto.
