...
- [X] T023 [P] [US5] Agregar `MYIVO_BACKUP_ENCRYPTION_KEY` y `MYIVO_BACKUP_REMOTE` al `.env.example` de la raíz (contracts/env-vars.md) — `MYIVO_BACKUP_RETENCION_DIAS` ya existe, sin cambio.
- [X] T024 [US5] Extender `scripts/backup-db.sh`: empaquetar `invoice_images` (vía `docker compose exec -T api tar ...`), combinar con el `.sql.gz` existente, cifrar con `age` usando `MYIVO_BACKUP_ENCRYPTION_KEY`, subir con `rclone` a `MYIVO_BACKUP_REMOTE`, y salir con error (sin dejar ningún artefacto sin cifrar) si falta cualquiera de las dos variables — contracts/backup-restore.md. Cifrado simétrico vía `age -e -i <archivo-identidad>` (no passphrase interactiva, no recipient/public-key aparte) — un solo secreto sirve para cifrar y descifrar, coherente con FR-018a.
- [X] T025 [US5] Crear `scripts/restore.sh`: descarga (si aplica)/descifrado con `age -d`/destar/restauración de `pg_dump` + imágenes; verificación de que el destino está vacío antes de escribir, exigiendo `--forzar` si no lo está — contracts/backup-restore.md. Las imágenes se restauran con `docker compose run --rm -T --no-deps --entrypoint sh api` (reusa el mismo volumen que declara el servicio real sin arrancar su entrypoint, para no disparar `prisma migrate deploy` antes de que el paso de la BD termine).
- [X] T026 [P] [US5] `scripts/backup-db.sh`: escribir `backups/ultimo-estado.json` (`{fecha, resultado, mensaje}`) en cada corrida, éxito o fallo — insumo de T041 (US8) y del propio `GET /health`. Implementado con un `trap` en `EXIT` — cubre tanto los `exit 1` explícitos como cualquier fallo inesperado bajo `set -e`.
- [X] T027 [P] [US5] `apps/api/src/app.controller.ts`: extender `GET /health` para leer `backups/ultimo-estado.json` e incluir `ultimoRespaldo` en la respuesta (contracts/health.md; `null` si el archivo todavía no existe) — FR-021. **Detalle no anticipado en data-model.md/research.md**: `backup-db.sh` corre en el host, `GET /health` corre dentro del contenedor `api` — sin un bind mount (`./backups:/backups:ro`, agregado a `docker-compose.yml`) el contenedor no podría leer el archivo del host.
- [X] T028 [P] [US5] Reescribir la nota "pendiente" de copia externa en `README.md` § Despliegue, enlazando a `contracts/backup-restore.md`/`quickstart.md` en vez de duplicar el procedimiento.
- [X] T029 [US5] Ejecutar `quickstart.md` § 6 completo, incluido el simulacro de restauración contra un entorno desechable (FR-020 exige que se haya probado al menos una vez de punta a punta) — confirma SC-005/SC-006. **Probado de punta a punta con Docker real**, no simulado: se instalaron `age`/`rclone` localmente, se creó un proyecto de Docker Compose aislado (`COMPOSE_PROJECT_NAME` distinto, volúmenes propios, nunca tocó los datos reales de desarrollo) con una factura + una imagen de prueba sembradas a mano. Se corrió `backup-db.sh` de verdad (subida a un directorio local usado como "remoto" de `rclone`, sin necesidad de una cuenta real en la nube): produjo el `.tar.age`, lo subió, y escribió `ultimo-estado.json` con `resultado: "ok"` — confirmado también vía `GET /health`. Se probó el camino de fallo (comentando la llave en `.env`): salió con código 1, mensaje claro, `resultado: "fallido"`, y ningún archivo sin cifrar. Para el simulacro de restauración se destruyeron los volúmenes del proyecto aislado (no los reales) y se movió el `.tar.age` local fuera del camino, forzando a `restore.sh` a descargarlo del "remoto" — el resultado fue exacto: la factura y la imagen de prueba volvieron idénticas (mismo contenido de archivo, mismo conteo), incluidas las políticas RLS del esquema. También se confirmó que `restore.sh` rechaza sobrescribir una base ya poblada sin `--forzar`. Limpieza completa al terminar — cero rastros de las pruebas.
...
- [X] T038 [US7] `apps/api/prisma/schema.prisma`: agregar `@@index([usuarioId, eliminadaEn, fechaHoraCompra])` y `@@index([usuarioId, cufe])` al modelo `Factura` (data-model.md § 2). Aplicado limpio sobre T030 (US6, ya en la misma rama) — sin conflicto real.
- [X] T039 [US7] Generar la migración de Prisma para los dos índices B-tree de T038, y agregar a mano en esa misma migración el índice GIN trgm (`CREATE INDEX ... ON facturas USING gin ("comercioNombreNormalizado" gin_trgm_ops)`) — Prisma no modela GIN trgm nativamente (mismo patrón ya usado para las políticas RLS). Generada con `--create-only` primero (mismo patrón que la migración de RLS) para editar el SQL antes de aplicarla una sola vez, sin drift de checksum.
- [X] T040 [US7] Ejecutar `quickstart.md` § 8: `\d facturas` en `psql` confirma los 3 índices nuevos; medir tiempo de respuesta de listado filtrado y de detección de duplicados — SC-007. **Probado con datos reales a escala**: se sembraron 5000 facturas sintéticas bajo una cuenta de prueba (sin tocar los datos reales) y se corrió `EXPLAIN ANALYZE` sobre las 3 consultas reales del código. Hallazgos honestos, no solo "funciona":
  - CUFE exacto (`detectarYRegistrar`): usa `facturas_usuarioId_cufe_idx` de inmediato, 0.24ms.
  - Listado filtrado por fecha: con un filtro angosto (una semana) usa `facturas_usuarioId_eliminadaEn_fechaHoraCompra_idx`, 0.33ms; con un filtro ancho (un año completo, ~22% de la tabla) el planner de Postgres elige un Seq Scan a propósito — a esta escala es genuinely más barato que saltar por el índice, comportamiento correcto del optimizador, no un índice roto (igual, 1.9ms de punta a punta).
  - Búsqueda difusa de comercio: la consulta original (`similarity() > umbral`) **no** usaba el índice GIN nuevo — `similarity()` como función no es index-aware, solo el operador `%` lo es. Se corrigió `duplicate-matching.service.ts` agregando la condición `"comercioNombreNormalizado" % ${...}` (redundante con el filtro exacto que ya existía, nunca cambia el resultado — el umbral de `%` es más laxo que `UMBRAL_SIMILITUD_COMERCIO`) — confirmado con `EXPLAIN ANALYZE` que ahora sí usa `Bitmap Index Scan` sobre el índice GIN. Sin este hallazgo, el índice GIN hubiera quedado sin usarse en la práctica. Limpieza completa de los datos sintéticos al terminar; suite de tests (dominio + e2e) sigue en verde.
...
- [X] T041 [US8] `apps/api/src/app.controller.ts`: extender `GET /health` con una verificación real de base de datos (`SELECT 1` vía Prisma) en el campo `baseDeDatos` — se suma al campo `ultimoRespaldo` que T027 (US5) ya había agregado; ambos son aditivos (contracts/health.md) — FR-027. Inyecta `PrismaService` directo (sin RLS: este endpoint no tiene sesión de usuario), `try/catch` propio para que un fallo de conexión nunca tumbe la respuesta HTTP misma.
- [X] T042 [US8] Auditar `apps/api/src/common/filters/global-exception.filter.ts`, `apps/api/src/common/logger/json-logger.service.ts` y los mensajes de error de `scripts/backup-db.sh`/`scripts/restore.sh` (T024/T025) para confirmar que ningún secreto (`*_SECRET`, `*_API_KEY`, `DATABASE_URL`, `MYIVO_BACKUP_ENCRYPTION_KEY`) puede llegar a una línea de log o a una respuesta HTTP — corregir cualquier punto donde sí pueda pasar (FR-028). **Auditoría con prueba empírica, no solo lectura de código**: se forzó una conexión real a Postgres con una contraseña falsa (`password authentication failed`) y con el puerto cerrado (`connection refused`) — en ningún caso el `.message`/`.stack` del error resultante de Prisma contiene la contraseña, confirmado programáticamente comparando el texto completo del error contra el valor secreto usado. Revisión del resto: `GlobalExceptionFilter`/`JsonLoggerService` solo extraen `.message`/`.stack` de cualquier error (nunca serializan el objeto completo, acotando la superficie de fuga a lo que ya se confirmó limpio); ningún endpoint recibe secretos por query string (revisado cada `@Query()` del proyecto); los SDK de proveedores de extracción (revisado `@anthropic-ai/sdk`) solo adjuntan las cabeceras de la *respuesta* del servidor al error, nunca las de la *request* saliente (que es donde vive el API key) — y de todos modos el código nunca lee esa propiedad. En los scripts, `MYIVO_BACKUP_ENCRYPTION_KEY` solo se usa para el chequeo `-z` (nunca imprime el valor) y para escribirse en un archivo temporal `chmod 600` que el `trap` borra siempre — ningún mensaje de error (`MENSAJE_ERROR=`) interpola su valor; `POSTGRES_PASSWORD` ni siquiera aparece en los scripts (la autenticación ocurre dentro del contenedor); las credenciales del proveedor de objetos externo viven enteramente en la configuración propia de `rclone` (`rclone config`), nunca pasan por variables de entorno ni por código de MyIvo. **Sin cambios de código** — la auditoría no encontró ninguna fuga real, solo la confirmó ausente.
- [X] T043 [US8] Ejecutar `quickstart.md` § 9: detener Postgres, confirmar que `/health` responde con `baseDeDatos: "error"` (sin dejar de responder) y volver a levantar Postgres. **Probado con Docker real**: `docker compose stop postgres` → `/health` respondió `{"status":"ok","baseDeDatos":"error","ultimoRespaldo":null}` (nunca dejó de responder); `docker compose start postgres` → volvió a `baseDeDatos:"ok"` sin reiniciar el contenedor `api`.
...
- [X] T044 [P] Anotar en `README.md` § Stack que `age` y `rclone` son dependencias operativas nuevas (binarios, no paquetes npm) — nota corta, distinta de la reescritura de § Despliegue de T010/T028.
- [X] T045 [P] Correr `pnpm typecheck` y `pnpm lint` en todo el monorepo y confirmar que quedan limpios. Limpios ambos, en `apps/api`, `apps/web` y `packages/domain`.
- [X] T046 Ejecutar `quickstart.md` completo, de principio a fin y en orden, contra un VPS/entorno desechable — cierre final de SC-001 a SC-008 antes de dar la feature por terminada. **Corrido contra el stack completo local (postgres+api+caddy vía `docker compose up`), no contra un VPS real** — igual que T022, sigue pendiente un VPS real para confirmar SC-001 (HTTPS en <15 min) de punta a punta; ver `PENDIENTES.md`. Localmente: arranque limpio de los 3 servicios, `/health` en `ok` con `baseDeDatos`/`ultimoRespaldo` reportando estado real, y la suite completa de tests de `apps/api` (70 de dominio + 5 e2e de aislamiento) en verde contra Postgres real ya migrado.
