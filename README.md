...
- **Login social** (opcional, por proveedor — Google, Microsoft, GitHub): cada uno se activa solo si tiene su `CLIENT_ID`/`CLIENT_SECRET` configurados (ver `.env.example`). Google marca el correo como verificado en su propia respuesta OAuth, así que una cuenta creada por ahí entra directo, sin pasar por la verificación por correo; Microsoft (cuentas de trabajo) y GitHub (correo privado) no siempre garantizan un correo verificado, así que esas cuentas pueden quedar pendientes de verificar igual que un registro por correo/contraseña.
- **Passkeys y verificación en dos pasos (TOTP)**, opcionales, gestionados desde "Mi cuenta": un passkey (huella, rostro o PIN del dispositivo) reemplaza la contraseña al iniciar sesión; el segundo factor pide además un código de una app autenticadora (Google Authenticator, Authy, etc.) tras activarlo con un código de verificación y guardar los códigos de respaldo generados una sola vez. Ambos funcionan también en cuentas que solo se crearon por Google/Microsoft/GitHub (sin contraseña propia) — `allowPasswordless` en `auth.ts`.
  **Pendiente de probar de punta a punta** (implementado y con `tsc`/`eslint` limpios, pero aún sin verificar contra la app corriendo): registrar un passkey y usarlo para entrar (`Login.tsx` → "Ingresar con passkey"), activar 2FA (QR + confirmación con la app autenticadora), entrar con una cuenta que tiene 2FA activo (paso de código en `Login.tsx`), y usar un código de respaldo cuando se pierde el acceso a la app autenticadora.
