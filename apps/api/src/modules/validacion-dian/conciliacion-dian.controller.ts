...
export class ConciliacionDianController {
  ...
  async conciliar(@UploadedFile() archivo?: Express.Multer.File): Promise<ResumenConciliacionDian> {
    if (!archivo) {
    ...
    const facturasCandidatas = await this.facturaRepository.listarConCufe();
    ...
    for (const facturaId of resultado.facturaIdsConciliadas) {
      await this.validacionDianRepository.crear(facturaId, 'conciliacion', 'valido_vigente');
    }
